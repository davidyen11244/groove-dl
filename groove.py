...
htmlclient = ('htmlshark', '20130520', 'nuggetsOfBaller',
              {"User-Agent": _useragent, "Content-Type": "application/json", "Accept-Encoding": "gzip"})
jsqueue = ['jsqueue', '20130520', 'chickenFingers']
jsqueue.append(
    {"User-Agent": _useragent, "Referer": 'http://%s/JSQueue.swf?%s' %
     (URL, jsqueue[1]), "Accept-Encoding": "gzip", "Content-Type": "application/json"})  # Contains all the information specific to jsqueue
...
h["country"]["IPR"] = 0
h["privacy"] = 0
h["session"] = (''.join(random.choice(string.digits + string.letters[:6])
                for x in range(32))).lower()
...
# Generate a token from the method and the secret string (which changes
# once in a while)
...
def getToken():
    ...
    conn.request("POST", "/more.php", json.JSONEncoder()
                 .encode(p), htmlclient[3])
    _token = json.JSONDecoder().decode(
        gzip.GzipFile(fileobj=(StringIO.StringIO(conn.getresponse().read()))).read())["result"]
...
def getResultsFromSearch(query, what="Songs"):
    ...
    conn.request("POST", "/more.php?" +
                 p["method"], json.JSONEncoder().encode(p), htmlclient[3])
...
def getStreamKeyFromSongIDs(id):
    ...
    conn = httplib.HTTPConnection(URL)
    conn.request("POST", "/more.php?" +
                 p["method"], json.JSONEncoder().encode(p), jsqueue[3])
...
# Remove a song from the browser queue, used to imitate a browser, in
# conjunction with the one above.
...
# Mark the song as being played more then 30 seconds, used if the download
# of a songs takes a long time.
...
if __name__ == "__main__":
    ...
    # i = raw_input("Search: ") #Same as above, if you uncomment this, and
    # comment the first 4 lines this can be run entirely from the command
    # line.
    print "Searching for '%s'..." % i
    m = 0
    s = getResultsFromSearch(i)  # Get the result from the search
    l = [('%s: "%s" by "%s" (%s)' % (str(m + 1), l["SongName"], l["ArtistName"], l["AlbumName"]))
         for m, l in enumerate(s[:10])]  # Iterate over the 10 first returned items, and produce descriptive strings.
    ...
    songid = raw_input(
        "Enter the Song IDs you wish to download (separated with commas) or (q) to exit: ")
    if songid == "" or songid == "q":
        exit()  # Exit if choice is empty or q
    inputtedIDs = songid.split(',')

    # songid = eval(songid)-1 #Turn it into an int and subtract one to fit it
    # into the list index
    queueID = getQueueID()
    for curID in inputtedIDs:
        ...
        cmd = 'wget --post-data=streamKey=%s -O "%s - %s.mp3" "http://%s/stream.php"' % (
            stream["streamKey"], s[songid]["ArtistName"], s[songid]["SongName"], stream["ip"])
        p = subprocess.Popen(cmd, shell=True)
        # Starts a timer that reports the song as being played for over 30-35
        # seconds. May not be needed.
        markTimer = threading.Timer(
            30 + random.randint(0, 5), markStreamKeyOver30Seconds,
            [s[songid]["SongID"], str(queueID), stream["ip"], stream["streamKey"]])
        markTimer.start()
        try:
            ...
            # Delete the song
            os.remove('%s - %s.mp3' %
                      (s[songid]["ArtistName"], s[songid]["SongName"]))
            print "\nDownload cancelled. File deleted."
        markTimer.cancel()
        print "Marking song as completed"
        # This is the important part, hopefully this will stop grooveshark from
        # banning us.
        markSongDownloadedEx(stream["ip"], s[songid]
                             ["SongID"], stream["streamKey"])
        # Natural Exit
